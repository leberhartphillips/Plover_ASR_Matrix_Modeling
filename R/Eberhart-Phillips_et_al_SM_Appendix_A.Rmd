...
hatching_sex_ratio <-
  read.table("data/hatching_sex_ratio_data.txt",
             header = TRUE)

parental_care_data <-
  read.table("data/parental_care_data.txt",
             header = TRUE)
```
...
sample_sizes <- reshape::cast(filter(parental_care_summary, brood_period == "late") %>%
                              ...
                              dplyr::summarise(n = n_distinct(brood_ID)),
...
First, for each population, we tested for significant deviations in the hatching sex ratio from parity. We used a binomial mixed-effects model that included *nest_ID* as a random effect to control for the non-independence of siblings.
...
HSR_summary <-
  hatching_sex_ratio %>%
  dplyr::group_by(population, nest_ID) %>%
