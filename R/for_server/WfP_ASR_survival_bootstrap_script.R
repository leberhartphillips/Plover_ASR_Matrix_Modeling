#-----------------------------------------------------------------------------#
#              Matrix Modelling of ASR in White-fronted plover                #
...
plover_boot_WfP <- function(WfP_F_A, WfP_C) {
  ...
  out <- list(WfP_C_boot = WfP_C_boot, WfP_F_A_boot = WfP_F_A_boot)
}

calc_ASR_WfP <- function(plover_boot_list, num_boot) {
  ...
  {
    setwd("/home/luke/comparative_ASR/Bootstrap/RMark_output_files/WfP/") 
...
# bootstrapping
run_boot_WfP <- function(num_boot, WfP_F_A, WfP_C){
  plover_list_WfP <- plover_boot_WfP(WfP_F_A, WfP_C)
  Result <- calc_ASR_WfP(plover_list_WfP, num_boot)
}

niter <- 1000

WfP_boot_1000_full_p <- sapply(1:niter, run_boot_WfP, WfP_F_A, WfP_C)

# extract AIC_table_C
WfP_AIC_table_C <- do.call(rbind, lapply(seq(from = 1, to = niter * 4, by = 4), function(x) WfP_boot_1000_full_p[[x]]))
num_mods <- nrow(WfP_AIC_table_C)/niter
WfP_AIC_table_C$iter <- rep(1:niter, each = num_mods)

# extract AIC_table_F_A
WfP_AIC_table_F_A <- do.call(rbind, lapply(seq(from = 2, to = niter * 4, by = 4), function(x) WfP_boot_1000_full_p[[x]]))
num_mods <- nrow(WfP_AIC_table_F_A)/niter
WfP_AIC_table_F_A$iter <- rep(1:niter, each = num_mods)

# extract WfP_Survival_rates
WfP_Survival_rates <- do.call(rbind, lapply(seq(from = 3, to = niter * 4, by = 4), function(x) WfP_boot_1000_full_p[[x]]))
WfP_Survival_rates$iter <- rep(1:niter, each = 6)

# extract WfP_ASR
WfP_ASR <- sapply(seq(from = 4, to = niter * 4, by = 4), function(x) WfP_boot_1000_full_p[[x]])
WfP_ASR <- data.frame(ASR = unname(WfP_ASR), iter = 1:niter)

write.table(WfP_AIC_table_C, 
            file = "/home/luke/comparative_ASR/Bootstrap/Total_output/WfP_AIC_table_C.txt",
            sep = "\t", row.names = FALSE, 
            col.names = TRUE)

write.table(WfP_AIC_table_F_A, 
            file = "/home/luke/comparative_ASR/Bootstrap/Total_output/WfP_AIC_table_F_A.txt",
            sep = "\t", row.names = FALSE, 
            col.names = TRUE)

write.table(WfP_Survival_rates, 
            file = "/home/luke/comparative_ASR/Bootstrap/Total_output/WfP_Survival_rates.txt",
            sep = "\t", row.names = FALSE, 
            col.names = TRUE)

write.table(WfP_ASR, 
            file = "/home/luke/comparative_ASR/Bootstrap/Total_output/WfP_ASR.txt",
            sep = "\t", row.names = FALSE, 
            col.names = TRUE)